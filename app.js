...
app.post('/newList', function(req, res){
	...
	User.findByIdAndUpdate(req.session.userId, {$push:{lists: {title: req.body.listName}}},{upsert:true, new:true},function(err, obj){
        if(err){
                console.log(err);
        }else{
...
app.post('/lists/:id/newTask', function(req, res){
	if (!req.session.userId){
		...
		User.findOne({'_id':req.session.userId}, 'lists', function(err, result){
		  	...
		  	list.tasks.push({title: req.body.taskName});
		  	var listId = 0;
			...
			for(var i = 0; i < result[0]['lists'].length; i++){
				if(result[0]['lists'][i]['_id'].toString() === req.params.id){
					for(var j in result[0]['lists'][i].tasks){
						result[0].lists[i].tasks[j].date = moment(new Date(result[0].lists[i].tasks[j]))
							.format('YYYY-MM-DD');
					}
...
app.get('/main', function(req, res){
	if (req.session.userId){
		res.sendFile('app/views/webApp.html' , { root : __dirname});	
	}else{
